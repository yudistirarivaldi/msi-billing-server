  ...
  <div class="space-y-6 relative">
    ...
    <div class="glass rounded-[3rem] overflow-hidden flex flex-col border border-slate-200 dark:border-white/10">
      <div class="p-8 border-b border-slate-200 dark:border-white/5 flex flex-wrap justify-between items-center gap-4">
        <div>
          <h4 class="text-xl font-black text-[#D72821] tracking-tight">Teams Overview</h4>
        </div>
        <div class="relative w-64">
          <Search class="absolute left-4 top-1/2 -translate-y-1/2 w-3 h-3 text-[#D72821]" />
          <input v-model="searchQuery" type="text" placeholder="Search team..." class="w-full bg-white dark:bg-white/5 border border-[#D72821]/30 dark:border-white/10 rounded-xl py-2 pl-10 pr-4 text-xs text-slate-900 dark:text-white outline-none focus:border-[#D72821] transition-all shadow-sm" />
        </div>
      </div>
      <div class="overflow-x-auto">
        <table class="w-full text-left text-sm">
          <thead class="bg-slate-50 dark:bg-white/[0.02] text-[10px] text-slate-600 dark:text-slate-400 uppercase tracking-widest font-bold">
            <tr>
              <th class="p-4 pl-8 w-48">Team Code</th>
              <th class="p-4">Team Name</th>
              <th class="p-4">Department (Parent)</th>
              <th class="p-4 text-right pr-8">Actions</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-200 dark:divide-white/5">
            <tr v-for="dept in paginatedTeams" :key="dept.id" class="hover:bg-slate-50 dark:hover:bg-white/[0.03] transition-all group">
              <td class="p-4 pl-8 text-sm font-mono font-black text-[#D72821]">{{ dept.team_code }}</td>
            ...
            <tr v-if="paginatedTeams.length === 0">
              <td colspan="4" class="p-8 text-center text-slate-500">No teams found.</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div v-if="showModal" class="fixed inset-0 flex items-center justify-center z-50 p-4 bg-slate-900/20 backdrop-blur-sm animate-fade-in">
      <div class="modal-bg bg-[#111111] rounded-[2rem] w-full max-w-md overflow-visible shadow-2xl border border-white/10 transform transition-all">
        <div class="modal-header rounded-t-[2rem] p-6 border-b border-white/5 flex justify-between items-center bg-white/[0.01]">
          <h2 class="text-xl font-black text-[#D72821] tracking-tight">{{ form.id ? 'Edit Team' : 'Add Team' }}</h2>
        ...
        <form @submit.prevent="saveKodePp" class="p-6 space-y-5">
          <div>
            ...
            <input v-model="form.team_code" type="text" required class="modal-input w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm font-bold text-white outline-none focus:border-[#D72821]/50 focus:ring-4 focus:ring-[#D72821]/10 transition-all">
          </div>
          <div>
          ...
          <div>
            <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Parent Department</label>
            <div class="relative" v-click-outside="() => showDeptDropdown = false">
              <div 
                ...
                :class="form.department_id ? 'text-slate-900 dark:text-white' : 'text-slate-400 dark:text-slate-400'"
              >
                <span class="truncate">{{ selectedDeptName || 'Select Department' }}</span>
                <ChevronRight class="w-4 h-4 transition-transform duration-200" :class="{ 'rotate-90': showDeptDropdown }" />
              </div>
              
              <div v-if="showDeptDropdown" class="absolute z-50 w-full mt-2 bg-white dark:bg-[#1a1a1a] border border-slate-200 dark:border-white/10 rounded-xl shadow-xl max-h-60 overflow-hidden flex flex-col dropdown-bg">
                <div class="p-2 border-b border-slate-200 dark:border-white/5 sticky top-0 bg-white dark:bg-[#1a1a1a] z-10 dropdown-bg">
                  <div class="relative">
                    ...
                    <input 
                      ...
                      class="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg py-2 pl-9 pr-3 text-xs text-slate-900 dark:text-white outline-none focus:border-[#D72821] transition-all dropdown-search"
                      @click.stop
                    />
                  </div>
                </div>
                <div class="overflow-y-auto p-1 flex-1">
                  <div 
                    v-for="dept in filteredDeptsDropdown" 
                    :key="dept.id"
                    @click="selectDept(dept)"
                    class="dropdown-item px-3 py-2 text-xs text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-white rounded-lg cursor-pointer transition-colors flex items-center justify-between group"
                    :class="{'bg-[#D72821]/10 text-[#D72821] hover:bg-[#D72821]/20 hover:text-[#D72821] dark:hover:text-[#D72821]': form.department_id === dept.id}"
                  >
                    <span class="truncate"><span class="font-mono text-[10px] text-slate-400 dark:text-slate-500 group-hover:text-slate-500 dark:group-hover:text-slate-400 mr-2">{{dept.department_code}}</span>{{ dept.name }}</span>
                    <CheckCircle2 v-if="form.department_id === dept.id" class="w-3 h-3 text-[#D72821]" />
                  </div>
                  <div v-if="filteredDeptsDropdown.length === 0" class="p-4 text-center text-xs text-slate-500">
          ...
          <div class="pt-6 flex justify-end gap-3">
            ...
            <button type="submit" class="bg-[#D72821] hover:bg-[#b01e18] text-white px-5 py-2.5 text-xs font-black uppercase tracking-wider rounded-xl transition-all shadow-md shadow-[#D72821]/20">Save Team</button>
          </div>
        </form>
      </div>
    </div>
...
const apiBase = 'http://localhost:3000/api/master/teams';
const deptApi = 'http://localhost:3000/api/master/departments';
const departments = ref([]);
const teams = ref([]);
const showModal = ref(false);
const form = ref({ id: null, team_code: '', name: '', department_id: null });

const showDeptDropdown = ref(false);
const deptSearchQuery = ref('');

const filteredDeptsDropdown = computed(() => {
  if (!deptSearchQuery.value) return departments.value;
  const q = deptSearchQuery.value.toLowerCase();
  return departments.value.filter(d => 
    String(d.department_code).toLowerCase().includes(q) || 
...
const selectedDeptName = computed(() => {
  ...
  return dept ? `${dept.department_code} - ${dept.name}` : '';
});
...
// Filter and sort by team_code
const sortedTeams = computed(() => {
  let list = [...teams.value];
  if (searchQuery.value) {
    const q = searchQuery.value.toLowerCase();
    list = list.filter(d => 
      d.team_code.toLowerCase().includes(q) || 
      d.name.toLowerCase().includes(q)
    );
  }
  return [...list].sort((a, b) => a.team_code.localeCompare(b.team_code));
});

const totalPages = computed(() => Math.ceil(sortedTeams.value.length / itemsPerPage) || 1);
...
const paginatedTeams = computed(() => {
  ...
  return sortedTeams.value.slice(start, start + itemsPerPage);
});
...
const fetchDepartments = async () => {
  try {
    const res = await axios.get(deptApi, { withCredentials: true });
...
const openModal = (dept = null) => {
  if (dept) {
    ...
    form.value = { id: null, team_code: '', name: '', department_id: null };
  }
...
const saveKodePp = async () => {
  try {
    if (!form.value.team_code || !form.value.name) {
      showNotification('Please fill in all required fields.', 'error');
      return;
    }
    if (!form.value.department_id) {
      showNotification(`Please select a Parent Department.`, 'error');
      return;
    }
    ...
    if (payload.id) {
      ...
      showNotification('Team successfully updated!');
    } else {
      await axios.post(apiBase, payload, { withCredentials: true });
      showNotification('Team successfully created!');
    }
